#!/usr/bin/python3
#
# backtest_2.py: contains functions for backtest method 2 of crypto bot 
# version 2.
#
# Andrew Bishop
# 2022/03/1
...
def increment_long_index(long_kline, short_kline, long_interval, short_interval):
    
    long_time = int(long_kline['t'])
    short_time = int(short_kline['t'])
    
    long_interval = decode_interval(long_interval)
    short_interval = decode_interval(short_interval)
    
    return 1 if ((long_time + 60*long_interval) < short_time) else 0


def backtest_method_2(symbol, days=10, new=True, print_flag=False):
